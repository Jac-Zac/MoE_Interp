...
# small enough that each AtP backward pass fits at the full batch size, so atp-batch-size
# matches batch-size here (gpt-oss needs a smaller one). The gate-AtP grid cache is keyed by
# concept and n-prompts, so changing --n-prompts writes a separate file.
